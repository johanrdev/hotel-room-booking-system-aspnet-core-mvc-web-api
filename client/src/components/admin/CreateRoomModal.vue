    ...
    <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
      ...
      <form @submit.prevent="createRoom" class="space-y-4">
        ...
        <div>
          ...
          <select v-model="roomTypeId" id="roomType" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
    ...
    async createRoom() {
      try {
        const roomData = {
          ...
          roomTypeId: this.roomTypeId // Skicka rumstypens ID till backend
        };

        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="room in rooms" :key="room.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ room.roomType?.name || 'N/A' }}</td>
              <td class="px-6 py-4 whitespace-nowrap">{{ room.roomType ? '$' + room.roomType.price : 'N/A' }}</td>
  ...
  methods: {
    ...
    async updateRoom(room) {
      try {
        ...
        const roomDetailsResponse = await axiosInstance.get(`/admin/rooms/${updatedRoom.id}`);
        updatedRoom.roomType = roomDetailsResponse.data.roomType;
    ...
    async createRoom(room) {
      try {
        ...
        const roomDetailsResponse = await axiosInstance.get(`/admin/rooms/${newRoom.id}`);
        newRoom.roomType = roomDetailsResponse.data.roomType;
